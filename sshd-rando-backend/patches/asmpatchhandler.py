...
class ASMPatchHandler:
    ...
    def write_patch(
        ...
        # Convert negative values to unsigned bytes (e.g., -1 -> 0xFF)
        # In Python, bytes() requires 0-255, but YAML may contain signed values
        normalized_data = []
        for byte_val in data:
            if byte_val < 0:
                # Convert signed byte to unsigned (e.g., -1 -> 255)
                normalized_data.append(byte_val & 0xFF)
            else:
                normalized_data.append(byte_val)
        
        if relative_offset < offsets.get_rodata_offset():
            file_offset = relative_offset - offsets.get_text_offset()
            write_bytes(text_segment, file_offset, bytes(normalized_data))
        elif relative_offset < offsets.get_data_offset():
            file_offset = relative_offset - offsets.get_rodata_offset()
            write_bytes(rodata_segment, file_offset, bytes(normalized_data))
        else:
            file_offset = relative_offset - offsets.get_data_offset()
            write_bytes(data_segment, file_offset, bytes(normalized_data))
    ...
    def patch_all_asm(self, world: World, onlyif_handler: ConditionalPatchHandler):
        ...
        # Always copy/patch sdk file - it's required for the game to run
        print_progress_text("Setting up SDK file")
        if ASM_DEBUG_PRINT:
            ...
            # Copy unpatched SDK file when debug printing is disabled
            # (SDK doesn't usually need patches for Archipelago)
            write_bytes_create_dirs(self.sdk_nso_path, SDK_FILE_PATH.read_bytes())
            ...
            for value_index, value in enumerate(self.shop_data[shop_index]):
                if value != -1:
                    if (format := SHOP_ITEM_DATA_FORMATS[value_index]) is not None:
                        ...
                        # Handle None values by using empty string
                        if value is None:
                            value = ""
